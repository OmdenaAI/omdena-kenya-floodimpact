...
def get_model(p, pretrain_path=None):
    ...
    # Get backbone
    if p['backbone'] == 'resnet18':
        if p['train_db_name'] in ['euroSAT','cifar-10', 'cifar-20','LandCover','LandCoverMod']:
...
def get_train_dataset(p, transform, to_augmented_dataset=False,
    ...
    elif p['train_db_name'] == 'LandCover':
       print('loading train landcover dataset....')
       from  data.landcover import LandCoverDataset 
       dataset = LandCoverDataset(df_path = 'data/lancover_dataset_60.csv',transform = transform)
    
    elif p['train_db_name'] == 'LandCoverMod':
       print('loading train landcover dataset....')
       from  data.landcover_mod import LandCoverDataset 
       dataset = LandCoverDataset(df_path = 'data/lancover_dataset_60.csv',transform = transform)
...
def get_val_dataset(p, transform=None, to_neighbors_dataset=False):
    ...
    if p['train_db_name'] == 'euroSAT':
      print('loading val euroSAT dataset....')
      from  data.euroSAT import EuroSat 
      dataset = EuroSat(df,transform = transform)
...
def get_train_transformations(p):
    if p['augmentation_strategy'] == 'standard':
        ...
        return transforms.Compose([
            ...
            transforms.CenterCrop(**p['augmentation_kwargs']['center_crop']),
            transforms.GaussianBlur(**p['augmentation_kwargs']['gaussian_blur']),
            transforms.RandomAffine(**p['augmentation_kwargs']['random_affine']),
